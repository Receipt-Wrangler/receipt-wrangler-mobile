...
class BulkStatusUpdateCommand {
  ...
  static BulkStatusUpdateCommand? fromJson(dynamic value) {
    if (value is Map) {
      ...
      return BulkStatusUpdateCommand(
        ...
        receiptIds: json[r'receiptIds'] is List
            ? (json[r'receiptIds'] as List).cast<int>()
            : const [],
      );
    }
    return null;
  ...
  static Map<String, List<BulkStatusUpdateCommand>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = BulkStatusUpdateCommand.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
