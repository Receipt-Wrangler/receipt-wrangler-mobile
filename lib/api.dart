//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

library openapi.api;

import 'dart:async';
import 'dart:convert';
import 'dart:io';

import 'package:http/http.dart';
import 'package:intl/intl.dart';
import 'package:meta/meta.dart';

part 'api_client.dart';
part 'api_helper.dart';
part 'api_exception.dart';
part 'auth/authentication.dart';
part 'auth/api_key_auth.dart';
part 'auth/oauth.dart';
part 'auth/http_basic_auth.dart';
part 'auth/http_bearer_auth.dart';

part 'api/auth_api.dart';
part 'api/category_api.dart';
part 'api/comment_api.dart';
part 'api/dashboard_api.dart';
part 'api/feature_config_api.dart';
part 'api/groups_api.dart';
part 'api/import_api.dart';
part 'api/notifications_api.dart';
part 'api/prompt_api.dart';
part 'api/receipt_api.dart';
part 'api/receipt_image_api.dart';
part 'api/receipt_processing_settings_api.dart';
part 'api/search_api.dart';
part 'api/system_email_api.dart';
part 'api/system_settings_api.dart';
part 'api/system_task_api.dart';
part 'api/tag_api.dart';
part 'api/user_api.dart';
part 'api/user_preferences_api.dart';

part 'model/ai_type.dart';
part 'model/app_data.dart';
part 'model/associated_entity_type.dart';
part 'model/associated_group.dart';
part 'model/base_model.dart';
part 'model/bulk_status_update_command.dart';
part 'model/category.dart';
part 'model/category_view.dart';
part 'model/check_email_connectivity_command.dart';
part 'model/check_receipt_processing_settings_connectivity_command.dart';
part 'model/claims.dart';
part 'model/comment.dart';
part 'model/dashboard.dart';
part 'model/encoded_image.dart';
part 'model/feature_config.dart';
part 'model/file_data.dart';
part 'model/file_data_view.dart';
part 'model/file_data_view_all_of.dart';
part 'model/filter_operation.dart';
part 'model/get_new_refresh_token200_response.dart';
part 'model/get_system_task_command.dart';
part 'model/group.dart';
part 'model/group_filter.dart';
part 'model/group_member.dart';
part 'model/group_role.dart';
part 'model/group_settings.dart';
part 'model/group_settings_white_list_email.dart';
part 'model/group_status.dart';
part 'model/import_type.dart';
part 'model/item.dart';
part 'model/item_status.dart';
part 'model/login_command.dart';
part 'model/logout_command.dart';
part 'model/magic_fill_command.dart';
part 'model/notification.dart';
part 'model/ocr_engine.dart';
part 'model/paged_data.dart';
part 'model/paged_data_data_inner.dart';
part 'model/paged_group_request_command.dart';
part 'model/paged_group_request_command_all_of.dart';
part 'model/paged_request_command.dart';
part 'model/paged_request_field.dart';
part 'model/paged_request_field_value.dart';
part 'model/prompt.dart';
part 'model/prompt_all_of.dart';
part 'model/receipt.dart';
part 'model/receipt_paged_request_command.dart';
part 'model/receipt_paged_request_filter.dart';
part 'model/receipt_processing_settings.dart';
part 'model/receipt_processing_settings_all_of.dart';
part 'model/receipt_status.dart';
part 'model/reset_password_command.dart';
part 'model/search_result.dart';
part 'model/sign_up_command.dart';
part 'model/sort_direction.dart';
part 'model/subject_line_regex.dart';
part 'model/system_email.dart';
part 'model/system_email_all_of.dart';
part 'model/system_settings.dart';
part 'model/system_settings_all_of.dart';
part 'model/system_task.dart';
part 'model/system_task_all_of.dart';
part 'model/system_task_status.dart';
part 'model/system_task_type.dart';
part 'model/tag.dart';
part 'model/tag_view.dart';
part 'model/token_pair.dart';
part 'model/update_group_settings_command.dart';
part 'model/update_profile_command.dart';
part 'model/upsert_category_command.dart';
part 'model/upsert_comment_command.dart';
part 'model/upsert_dashboard_command.dart';
part 'model/upsert_item_command.dart';
part 'model/upsert_prompt_command.dart';
part 'model/upsert_receipt_command.dart';
part 'model/upsert_receipt_processing_settings_command.dart';
part 'model/upsert_system_email_command.dart';
part 'model/upsert_system_settings_command.dart';
part 'model/upsert_tag_command.dart';
part 'model/upsert_widget_command.dart';
part 'model/user.dart';
part 'model/user_preferences.dart';
part 'model/user_preferences_all_of.dart';
part 'model/user_role.dart';
part 'model/user_view.dart';
part 'model/widget.dart';
part 'model/widget_type.dart';


const _delimiters = {'csv': ',', 'ssv': ' ', 'tsv': '\t', 'pipes': '|'};
const _dateEpochMarker = 'epoch';
final _dateFormatter = DateFormat('yyyy-MM-dd');
final _regList = RegExp(r'^List<(.*)>$');
final _regSet = RegExp(r'^Set<(.*)>$');
final _regMap = RegExp(r'^Map<String,(.*)>$');

ApiClient defaultApiClient = ApiClient();
